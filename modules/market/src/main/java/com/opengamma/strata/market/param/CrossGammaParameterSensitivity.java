 ...
 * The {@code (i,j)} component is the sensitivity of the {@code i}-th delta for {@code parameterMetadata} to 
...
public final class CrossGammaParameterSensitivity
  ...
  */
   ...
   */
  @PropertyDefinition(validate = "notNull", builderType = "Map<MarketDataName<?>, List<? extends ParameterMetadata>>")
  private final Map<MarketDataName<?>, List<? extends ParameterMetadata>> order;
  ...
  */
  ...
  public static CrossGammaParameterSensitivity of(
    ...
    return new CrossGammaParameterSensitivity(marketDataName, parameterMetadata,
        ImmutableMap.of(marketDataNameOther, parameterMetadataOther), currency, sensitivity);
  }
   ...
   * The market data set is represented in terms of {@code Map<MarketDataName<?>, List<? extends ParameterMetadata>>}, 
   * which also defines the order of the sensitivity values.
   * <p>
   * The market data name identifies the {@link ParameterizedData} instance that was queried.
   * The parameter metadata provides information on each parameter.
   * 
  ...
  public static CrossGammaParameterSensitivity of(
      MarketDataName<?> marketDataName,
      List<? extends ParameterMetadata> parameterMetadata,
      Map<MarketDataName<?>, List<? extends ParameterMetadata>> order,
      Currency currency,
      DoubleMatrix sensitivity) {
  ...
  private void validate() {
    ...
    int nParamsTotal = 0;
    for (Entry<MarketDataName<?>, List<? extends ParameterMetadata>> entry : order.entrySet()) {
      nParamsTotal += entry.getValue().size();
    }
    if (col != nParamsTotal) {
  ...
  /**
   * Compares the key of two sensitivity objects, excluding the parameter sensitivity values and order.
   * 
  ...
  public CrossGammaParameterSensitivity getSensitivity(MarketDataName<?> name) {
    ...
    int colCt = indexAndMetadata.second().size();
    ...
    return CrossGammaParameterSensitivity.of(
        getMarketDataName(), getParameterMetadata(), name, indexAndMetadata.second(), getCurrency(),
  ...
  private Pair<Integer, List<? extends ParameterMetadata>> findStartIndexAndMetadata(MarketDataName<?> name) {
    ...
    for (Entry<MarketDataName<?>, List<? extends ParameterMetadata>> entry : order.entrySet()) {
      if (entry.getKey().equals(name)) {
        return Pair.of(startIndex, entry.getValue());
      }
  ...
  private CrossGammaParameterSensitivity(
    ...
    this.order = ImmutableMap.copyOf(order);
   ...
   * @return the value of the property, not null
   */
  public Map<MarketDataName<?>, List<? extends ParameterMetadata>> getOrder() {
    return order;
  }
  ...
  public static final class Meta extends DirectMetaBean {
     ...
     */
    @SuppressWarnings({"unchecked", "rawtypes" })
    private final MetaProperty<Map<MarketDataName<?>, List<? extends ParameterMetadata>>> order = DirectMetaProperty.ofImmutable(
        this, "order", CrossGammaParameterSensitivity.class, (Class) Map.class);
    /**
     ...
     * @return the meta-property, not null
     */
    public MetaProperty<Map<MarketDataName<?>, List<? extends ParameterMetadata>>> order() {
      return order;
    }
  ...
  private static final class Builder extends DirectFieldsBeanBuilder<CrossGammaParameterSensitivity> {
    ...
    private Map<MarketDataName<?>, List<? extends ParameterMetadata>> order = ImmutableMap.of();
    ...
    public Builder set(String propertyName, Object newValue) {
      switch (propertyName.hashCode()) {
        ...
        case -1169106440:  // parameterMetadata
          ...
          break;
        case 106006350:  // order
          this.order = (Map<MarketDataName<?>, List<? extends ParameterMetadata>>) newValue;
          break;
        case 575402001:  // currency
